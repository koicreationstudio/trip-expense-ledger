...
> **⚠️ 2026-09-23 体检提醒（第三十七轮，字号/圆角/字体部分仍然有效；色板部分已被第三十八轮推翻，见下）：这份文档从这里往下到「第三十二轮」为止，字号密度/圆角尺度/Fraunces→IBM Plex Serif 字体切换这几件事已经过时，不是当前权威规格，像素级权威来源是 `reference/artifact-v10-source.html` + 代码里的行内注释 + `PENDING-DECISIONS-trip-expense-ledger.md` 第九轮到第三十六轮。** 但下面 v3-v8 描述的**色板**（remy-thailand 暖色系十色，经「色彩强度柔和化」柔和过一版）——**这部分在 2026-09-23 第三十八轮被重新启用，是当前真正生效的颜色规格**，不是历史存档。详情见文末「第三十八轮」一节：2026-09-11 候选D把色板换成了近乎单色灰阶，`reference/artifact-v10-source.html` 的 `:root` 也在那之后跟着变成灰阶，第三十七轮体检发现这个转向从没跟 Remy 正式确认过，还跟本文档 v5-v7 反复论证的"不能像团队工具冷灰调"直接矛盾；Remy 确认要改回暖色系后，`tailwind.config.ts` 和 `reference/artifact-v10-source.html` 的色板已经一起改回下面 v3-v8 记录的暖色定案值。**读下面 v3-v8 时，色板 hex 值可以照抄，字号/圆角/字体不要照抄**（那几项以第三十二轮之后的记录为准）。 **⚠️ 2026-09-24 更新：上面这段关于"暖色是当前真正生效颜色规格"的表述已过时——色板已在 2026-09-24 第四十轮再次反转为灰阶，本横幅这段暖色权威性描述不再成立，最新状态请见文末「第四十轮：色板第三次反转，暖色 → 灰阶」一节，别只读到这里就下结论。**
...
> **【2026-09-23 第三十八轮补记】下面「色彩强度柔和化」「第六版」「第七版」这三节，
> 2026-09-23 22:50（round37 commit `6962d73`）曾经被整段删除过（112 insertions/365
> deletions），commit message 当时写的是"原样保留没删改"，跟实际改动不符——这是 round37
> 汇报里的一处真实失实，lifeos-pm 核实 `git show 6962d73` 后指出。这次（第三十八轮）选择
> 补回原文而不是改口径认错删除，原因很直接：Remy 这轮明确要求"色板改回暖色系"，而这三节
> 正是暖色最后一次被认真论证、算过 WCAG 对比度的定案记录（`ink #23232E`/`gold #B89E61`/
> `gold-dk #7E6630`/`gold-lt #F0E8D6` 这批当前 `tailwind.config.ts` 已经在用的暖色值，
> 直接来自这三节），删掉它们不但是失实，还会让"这批色值当初怎么算出来的"这个依据一起
> 消失。下面文字逐字照 round37 之前的版本原样还原，不做任何改写。**这三节现在重新是当前
> 权威状态**（因为色板已经改回来了，不再是"已废弃的历史存档"）——文件最前面第 3 行的
> 提醒横幅、以及第三十七轮体检那节的表格，都是在色板还是候选D灰阶时写的，读到那两处时
> 请对照最后「第三十八轮」一节看这轮色板复原的最新结论，不要只信旧提醒。

> **⚠️ 2026-09-24 更新（第四十轮）：上面这条补记里"这三节现在重新是当前权威状态"、
> "这批当前 `tailwind.config.ts` 已经在用的暖色值"这两句表述已经过时。色板已在
> 2026-09-24 第四十轮再次被推翻，从暖色改回灰阶——`tailwind.config.ts` 现在用的
> 不再是这三节记录的暖色值。这三节原文继续保留（同样的理由：删掉会丢失当初怎么
> 算出这批值的依据），但现在只是历史存档，不是当前权威状态。当前真正生效的色板
> 请见文末「第四十轮：色板第三次反转，暖色 → 灰阶」一节。**
...
2. **色彩单调**——整张卡只有一个色相（ink 藏青），文字只有白色/emerald/red 三种，跟"视觉丰富化"这个第三版就定的调性方向本身有落差——不是要推翻 ink 底色，是这张卡除了底色之外没有第二层视觉语言。
...
- `本位币 {baseCurrency}`：`text-xs text-slate-500`(12px)——比第四版定的"副信息 10px"档还大 2px，且用的是没接入色板的 Tailwind 默认灰
- 导航链接（行程主页/结算/支付方式/邀请管理）：`text-sm`(14px)，比正文常规字号还大，跟其它模块的紧凑基准完全脱节
...
| 导航链接 | `text-sm`(14px) | `text-[12.5px] text-muted`，选中态 `text-ink font-semibold` |
| 分隔线 | `border-slate-200` | `border-sand`——接入色板，不再是 `slate` 孤岛 |

**触控热区保留**：导航链接字号收紧不等于点击热区跟着收紧——继续保持 `min-h-[44px]`（Apple HIG 最小触控目标），紧凑的是视觉尺寸，不是可点击范围，这条第四版就定过，这次延续。
...
- **remy-thailand 参照决定本身**——不重新讨论"要不要学 remy-thailand"，这轮是在已经确认的方向上补质感，不是重新选方向
- **圆角/字体家族**（pill/14px/22px、Fraunces+Inter）——这次没有证据表明这两件事有问题，Remy 反馈的是"没感觉"和"字号"，不是圆角或字体本身

## 要不要跟进 suimuse 这条参照路线？
...
## A. 这次要不要参照 team-board？三个维度分开判断，不是整体照搬或整体拒绝
...
## B. 根因：`body` 默认文字色从没接过色板

实读 `app/layout.tsx` + `app/globals.css`：

```css
/* app/globals.css */
:root { --foreground: #0f172a; }  /* Tailwind slate-900，没有接色板 */
```
...
## C. 三档层级怎么落地（延续 A 节"采纳纪律不采纳具体 hex"的判断）
...
Remy 这句反馈里其实还嵌了一个新功能请求——"参照 team-board 的...排版"这句话的上下文是在对比"每个系统都有自己的排版"，隐含她可能想要在 trip-expense-ledger 里也能像切 team-board 部门看板一样快速切换行程。查了现有代码：`app/page.tsx` 首页确实列出"我的行程"清单，但**进入某个行程之后，`[tripId]/layout.tsx` 导航栏里没有任何"切回行程列表"或"切换到其它行程"的入口**，唯一的路径是点浏览器返回键或手动改 URL。这是一个真实缺口，不是这次色彩反馈的核心，但顺手记录：

**建议新增**：导航头 `trip.name` 这颗文字本身升级成一个下拉触发器（不新增视觉元素占位，复用已有的身份文字位置），点击展开一个轻量下拉列表，列出该用户名下其它行程（复用 `app/page.tsx` 已经在查的数据），选中即跳转。视觉规格：

- 下拉容器：`rounded-xl border border-sand bg-paper shadow-card`（跟上面新增的 `shadow-card` token 是同一个，不新造）
...
- 触发方式：点击 `trip.name` 展开/收起，旁边加一个小箭头 `▾`（跟已有 `.detail-toggle` 用同一个符号语言，不新引入图标）
- 底部一行"＋ 创建新行程"：`tap-link` 风格，链接到 `/trips/new`

这条是这次任务范围之外的新功能建议，**不强制这轮做**，但既然读代码时发现了这个真实缺口，一并记录供排期参考。
...
- 十色板 hex 值、圆角尺度、emoji 图标语言、Fraunces/Inter 字体分工——不重新讨论

**这次新加的（第七版特有）**：

- `body`/`layout.tsx` 根因修复（`text-slate-900`→`text-ink`）
- 全站 24 处散落 `text-slate-*` 归位到 `text-ink`/`text-muted` 两档
- `border-slate-200`→`border-sand` 全站统一
- `shadow-card` 新 token，普通卡片接光影
- 三个被点名页面 + 首页共 4 处 h1 统一成 `text-base text-ink`
- 顺手记录的新功能建议：行程切换下拉入口（不强制这轮做）
...
> **【2026-09-23 第三十八轮更新】这一节记录的是体检当时（灰阶仍在生产环境）的状态。Remy 看到这份体检后已经拍板"改回暖色系"，`tailwind.config.ts`/`app/globals.css`/`reference/artifact-v10-source.html` 的色板已经在第三十八轮改回下面表格"v3-v8 文档值"那一列，不再是"当前代码真实值"那一列——下面表格原样保留（留作这次转向的证据链，不改写），但读的时候要知道现在实际生效的是左边那列，不是右边。详细改动清单见文末「第三十八轮」一节。**
...
# 第三十八轮：色板改回暖色系 + 结算页 FAB 遮挡排查 + 钱包余额联动排查（2026-09-23）

背景：Remy 看过上面「第三十七轮」体检报告后，针对四件事逐条拍板/追加要求，trip-expense-ledger-pm 这轮处理。

## 一、色板改回暖色系——已完成，依据是上面「色彩强度柔和化」+「第七版」两节的定案值

**不是凭感觉挑一个暖色**：直接采用上面「色彩强度柔和化（2026-09-08）」一节最后一次经过 WCAG 对比度实算、并被「第七版」确认继续有效的那批暖色值——这是 v3 remy-thailand 原始十色板做过一轮柔和化之后的"最终定案态"，不是最早最艳的那版，也不是随手挑的新颜色。

| Token | 改回的值 | 来源 |
|---|---|---|
| `ink` | `#23232E` | 色彩强度柔和化：`#1A1A2E`→`#23232E`，红字(该付)在此底色对比度 3.22:1，是全套色板最紧的约束项 |
| `gold` | `#B89E61` | 同上，`#C9A24B`→`#B89E61` |
| `gold-dk` | `#7E6630` | 同上，`#8B6914`→`#7E6630`（三阶金色里柔和化降幅最大的一个，是"标题太艳"的真正成因） |
| `gold-lt` | `#F0E8D6` | 同上，`#F5EDD0`→`#F0E8D6` |
| `paper` | `#FEFCF7` | v3 原始值，未被柔和化那轮改过 |
| `sand` | `#EDE8DA` | 同上 |
| `muted` | `#8A7A6A` | 同上——候选D落地时把它跟 `gold-dk` 并成了同一个灰色号(`#6E6E6C`)，这次两者拆开各自改回暖色定案值 |

`accent-circle`（头像圈/图标圈）跟着 `gold` 同值改回 `#B89E61`；`hero-gradient`/`boxShadow.hero`/`boxShadow.card` 三个引用 ink RGB 的 token 一并改回 `35,35,46`（`#23232E` 的 RGB）；`hero-gradient` 的线性渐变主体改用「第六版」已验证过 WCAG 对比度的纯 ink 色相三段式 `#2A2A38→#23232E→#1B1B24`，**没有沿用候选D那版"渐变起点用 gold-dk"的结构**——实算过 `negative-dk`(`#E6B1A8`) 在新 `gold-dk`(`#7E6630`) 上对比度只有 2.93:1，没过 WCAG 大字号 3:1 门槛，重蹈了候选D自己在 2026-09-12 修过的同一类问题（当时是 `positive-dk` 在 Mountain Mist 上只有 2.12:1），所以这次直接换回纯 ink 色相的渐变主体，只在装饰性的右上角光晕层（不叠文字）里保留暖金色调 `rgba(184,158,97,.30)`。全仓库 30 处硬编码 `rgba(164,163,160,...)`（旧 gold 灰阶 RGB）和 1 处 `rgba(55,55,54,...)`（旧 ink 灰阶 RGB）一并改成新 RGB 值，逐文件 grep 核对过没有漏网。

**`reference/artifact-v10-source.html` 的 `:root` 色板也一并改回暖色**——这份文件是全项目"逐 token 核对 Artifact 字面规格"时反复当权威源头用的（round18/20/26/32 都这么做过），2026-09-11 候选D落地时它的色板也跟着变成了灰阶，一直没人发现也没人同步改回来；这次不改回来的话，以后任何人再做一次"逐 token 核对"都会把暖色误判成"漂移"、又改回灰阶——这正是这次色板拉锯的根本机制（详见文末「四」）。

**不动的颜色**：`coral`(`#991B14`)/`negative`/`positive`(及各自 `-dk` 变体)/`ok`/`live`/`seafoam` 这几个候选D时期定的语义色本身不是灰阶（是真实带色相的红/绿/蓝），不在"变灰"问题范围内，原样保留。

**verified**：`npx tsc --noEmit` 通过，`npx eslint` 通过，109 个单测全过；ui-auditor 真机走查结果见文末「五」。

## 二、结算页面 FAB 遮挡最后一行净值——排查结论：**代码结构本身正确，round37 截图证据大概率是 Playwright 全页截图工具的渲染假象，不是真实用户会遇到的 bug**，已用真机走查交叉验证

**代码结构核实**（`app/trips/[tripId]/record-expense-bar.tsx` + `app/globals.css` `.action-bar`/`.action-bar-reserve`）：这个"记一笔消费"入口早在 2026-09-12（第三轮同类反馈之后）就已经从"贴右下角悬浮胶囊"重构成"占一条自己的横带 + 页面容器预留同高度底部空白"，`--action-bar-h` 单一变量同时驱动两边，理论上不可能出现"滚到底时内容被永久盖住"——这是这个结构存在的唯一目的。`app/trips/[tripId]/settlement/settlement-body.tsx` 的展开/收起逻辑只是往 DOM 里插入更多 `<li>`，不影响这条底部 reserve padding 的计算方式。

**round37 截图（`08_settlement_desktop_expanded.png`/`09_settlement_mobile_expanded.png`）复核**：两张图里"htoo"净值行被按钮压住的同时，图片下方仍然能看到"转账清单"整节内容（htoo→remy 转账行、转账进度文字、"等所有转账确认收款后"按钮）——说明这不是"滚到页面真正的底部"，按钮压住的是页面中段。真实用户遇到固定定位的底部操作条时，中段内容被暂时挡住是正常现象（任何 fixed 底部栏都这样，比如微信输入框），只要继续往下滚就能看到；这跟"滚到底后仍然被永久盖住"是两回事，后者才是需要根治的问题。这两张截图的具体拍摄手法（是否为 Playwright fullPage 截图模式，该模式已知会把 `position:fixed` 元素渲染在跟原始视口高度对应的固定像素位置、而不是文档真正的末尾）没有在这轮追查到源头，但真机走查（见「五」）用视口截图（非整页）+ 真实滚动到底测试，两种情况（1 笔明细/多笔明细全部展开）都确认最后一行净值完全可见、没有被压住。

**结论**：这次没有改动 `record-expense-bar.tsx`/`.action-bar` 的任何结构（因为找不到需要改的地方，改了反而可能引入新问题）。如果 Remy 之后在真机上（不是截图里）仍然遇到"滚到底还是被压住"的情况，请带上"是否已经滚到底部（下面还有没有更多内容）"这个细节反馈，会更容易定位。

## 三、钱包余额联动排查——真实数据坐实：**联动逻辑本身已经存在且工作正常，问题是"仅未来生效、不回溯"这条边界从没在界面上说清楚，也没有入口指引**

**(a) "创建没反应"不是技术 bug**：直接查了 D1 真实数据（trip `f78a6b5e-8612-4097-8bfd-88a5db664045`），Remy 刚创建的"现金"钱包（`id=872246bc...`）确实创建成功，`currency=HKD`、`payment_method_id` 也确实正确绑定到她的"现金"支付方式（`id=a31f5c30...`）——表单、绑定、`router.refresh()` 全部按预期工作，不是"输入框没生效"或"创建后没刷新"。

**(b) 真正的根因，两层认知落差叠加**：
1. **起始余额输入框确实被拿掉了**（round13/Artifact V10 拍板"改由『支付方式』页『设置当前余额』统一承担"），但钱包卡本身完全没有任何指向那个功能的入口或提示，Remy 不知道要去哪里设。这次已修（见下）。
2. **`app/api/trips/[tripId]/expenses/route.ts` 第 95-118 行的"记账自动扣钱包余额"逻辑，只对绑定之后新记的消费生效，不会回溯计算钱包成立之前已经记过的消费**——这是代码注释里明确写的"v1 明确的简化边界"，不是本轮才发现的疏漏。查了 Remy 那 5 笔真实"现金"消费（咖啡/云吞面/雪糕/酒店tax/taxi，都绑定同一个"现金"支付方式），全部的 `created_at` 都早于钱包的 `created_at`（钱包是 2026-09-23 才建的，5 笔消费是 09-15 到 09-18 之间记的）——所以联动这条链路一次都没触发过，不是没做，是这几笔消费从时间上就赶不上。

**上一轮"钱包余额是独立字段、不会因为记消费自动变"的诊断是不完整的**——联动逻辑确实存在（`wallets.paymentMethodId` 绑定 + 币种精确匹配时会自动扣），只是有"仅未来生效"这个边界，上一轮没有查到这一层，说"两者完全独立"不准确，这次订正。

**(c) 这轮已经做的修复**（不是产品判断，是打通已经存在的发现路径）：
- `app/trips/[tripId]/wallet-grid.tsx`：钱包卡下方新增一行常驻提示 + 深链，非空状态下都会显示："余额不会跟着消费记录自动变——只有钱包'绑定支付方式'之后新记的消费才会自动扣，已经记过的不会补算。去'支付方式'手动设置余额 →"，链接到 `/trips/{tripId}/payment-methods?openBalance=1`。
- `app/trips/[tripId]/payment-methods/page.tsx` + `payment-methods-manager.tsx`：接住 `?openBalance=1` 深链，落地直接展开"⚙设置当前余额"面板并自动滚动到该区块（面板在页面最下面，光展开不滚过去等于白做），同时把面板说明文字从"这里改的是钱包余额不是支付方式配置"扩充成把"仅未来生效、不回溯"这条边界也讲清楚。

**(d) 没有做的事，是一个真实的产品范围决定，留给 Remy 表态**：要不要做"回溯计算"——即绑定钱包时（或事后）把这个支付方式名下、钱包建立之前的历史消费也补算进余额里。两个方向的利弊：
- **方案 A：不做回溯，维持现状（只是把边界讲清楚）**——优点：改动小、风险低，不会有"回溯范围从哪天算起/要不要排除已经手动设过的余额/多币种消费怎么处理"这类新的边界问题；缺点：Remy 每次新建一个已经有历史消费的钱包，仍然要手动去"设置当前余额"把这笔账拉平一次，只是这次至少知道要去哪里拉。
- **方案 B：新建钱包或首次绑定支付方式时，提供"要不要把这个支付方式名下的历史消费也算进来"的一次性选项**——优点：真正解决"记了很多现金消费，钱包却是 0"这个直觉落差；缺点：需要新设计（回溯范围怎么定、算法怎么防止跟"手动设置余额"重复计算、绑定关系变更/解绑之后历史算过的部分要不要回滚）——这是一次不小的架构改动，不建议在这轮顺手做。

这次先做方案 A（已完成），方案 B 作为悬案记录，等 Remy 看完这条说明后决定要不要排期。

## 四、DESIGN-BRIEF.md「原样保留」失实订正——选择方案(a)：补回被删的三节内容，不改口径

round37 commit（`6962d73`）的 commit message 写的是这份文档"原样保留没删改"，但 `git show 6962d73 -- DESIGN-BRIEF.md` 显示实际是 112 insertions / 365 deletions——"色彩强度柔和化（2026-09-08）"/"第六版"/"第七版"共约 365 行被整段删除，不是浓缩、不是移到别处。lifeos-pm 核实后指出这一处失实，这轮要求二选一订正。

**选了方案(a)：补回原文当历史存档，不是方案(b)改口径认错删除**。理由很直接：这三节正是暖色最后一次被认真论证、算过 WCAG 对比度的定案记录——上面「一、色板改回暖色系」用到的 `ink #23232E`/`gold #B89E61`/`gold-dk #7E6630`/`gold-lt #F0E8D6` 这批值，以及 hero 渐变/阴影的具体 rgba 数值，全部直接来自这三节。选方案(b)（只改措辞）没法解决"这批色值当初怎么算出来的"这个依据凭空消失的问题；选方案(a)一次性解决两件事——既订正了"原样保留"这句失实描述（现在是真的原样保留了），又把这次改色的依据留在了文档里，不用重新推导一遍。补回内容逐字照 round37 之前的版本（`git show 6962d73~1:DESIGN-BRIEF.md`）原样还原，没有做任何改写；文档最前面的提醒横幅和「第三十七轮」体检那节的对比表格都保留原样（作为"曾经发生过这个矛盾"的证据链），只在旁边加了指向这节的更新提示，没有删改原有文字。

## 五、验证方式（全部用真实行程「🇭🇰2026香港」，`trip id=f78a6b5e-8612-4097-8bfd-88a5db664045`）
...
# 补记（2026-09-24，PENDING-DECISIONS 第四十九轮）：第八版"不该做的事"第 5 条已被 Remy 明确推翻

第八版（上面那一节，2026-09-08）"不该做的事"第 5 条写过："不要求把 `settlement/page.tsx` 的'分组盒子+行'结构改成跟 `payment-methods`/`invites` 一样的'逐行卡片'结构"，理由是"关系类清单（谁欠谁/净值/转账）用分组盒子，记录类清单用逐行卡片，是两种内容性质决定的两种呈现方式"。

这条决定形成时**没有回头核对 Artifact V10 权威源里净值清单本身的真实 HTML 结构**，是基于"关系类 vs 记录类"这个抽象分类推出的判断。2026-09-24 Remy 亲自拿生产截图和设计稿逐屏肉眼比对（PENDING-DECISIONS 第四十九轮），逐字核对了 `reference/artifact-v10-source.html` 第 846-871 行，确认权威源里每个人（remy/alex/ben）本来就是各自一个独立的 `<div class="list"><div class="p-row">...</div></div>`，不是一个分组盒子包住所有人——第八版这条决定的前提本身跟权威源不符。

`settlement-body.tsx` 的净值清单已经在第四十九轮改成"每人独立胶囊卡"，第八版这条"不该做的事"第 5 条**已作废**，不再适用。转账清单（`transfers`）这部分继续维持"一个分组盒子"结构没有动——这部分本来就没有被 Remy 这次指出问题，第八版对转账清单的判断继续有效，只有净值清单这一半被推翻。
