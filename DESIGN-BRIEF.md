...
> **⚠️ 2026-09-23 体检提醒（第三十七轮）：这份文档从这里往下到「第三十二轮」为止，绝大部分内容已经过时，不是当前权威规格。** 先跳到文末「第三十七轮：全面体检」读完再回头看下面的历史内容——下面 v3-v8 描述的色板（remy-thailand 暖色系十色）、字体（Fraunces）在 2026-09-11/09-13 被 Remy 亲自拍板的两次改动整体推翻，现在唯一的像素级权威来源是 `reference/artifact-v10-source.html` + 代码里的行内注释 + `PENDING-DECISIONS-trip-expense-ledger.md` 第九轮到第三十六轮。下面的文字**留着当历史存档**（论证逻辑、否决理由这些大部分依然成立，只是具体色值/字体已经被替换），不要照抄里面的十六进制颜色值或 Fraunces 字体名去改代码。
...
# 第三十七轮：全面体检（2026-09-23，Remy 反馈"感觉有点卡，怀疑视觉协调度没被重新审视过"）

## 体检结论一句话

**上面 v3-v8（"第三版"到"第八版"）描述的十色暖色板（remy-thailand ink/gold/sand/paper 等）和 Fraunces 字体，已经在 2026-09-11（配色候选 D 落地）和 2026-09-13（Remy 拍板确认"Artifact Version 10"为权威方案）被 Remy 本人的两次真实决定整体推翻，但这份文档从 2026-09-08（v8）到 2026-09-23（第三十二轮）之间，从没有被系统性重写来反映这个转向。** 第三十二轮只补了圆角/spacing 两个窄口径 token，v3-v8 那 900 多行文字（色板 hex 值、Fraunces 纪律、"参照 remy-thailand"整套论证）留在文档里，任何人从头读这份文档会得到一幅跟当前真实代码完全对不上的画面。这正是②③两个"卡住待拍板"疑案的真正成因——不是没人拍板，是拍板结果没被写回这份文档，散落在 `tailwind.config.ts`/`app/globals.css` 的行内注释和 `PENDING-DECISIONS-trip-expense-ledger.md` 第九轮到第三十六轮里。

## 一、②③两个悬案：都已经解决，不是真悬案

### ② 数字字体——已解决，现行字体是 IBM Plex Serif，不是 Fraunces

**证据链**（全部来自当前真实代码，不是猜的）：
- `reference/artifact-v10-source.html`（本项目自己认定的权威静态方案，Round 32 用它钉过圆角/spacing）第 19 行：`--font-serif: 'IBM Plex Serif', Georgia, serif;`——方案本身从根上就没有 Fraunces。
- `app/layout.tsx` 第 2/11-14 行：`import { Inter, IBM_Plex_Serif, IBM_Plex_Mono } from 'next/font/google'`，注释原话"2026-09-13：Remy 拍板从 Fraunces 换成 IBM Plex Serif（配色对比稿定案版本），只换这一个 next/font 声明 + tailwind.config.ts 的 serif token，全站 14 处"。
- `tailwind.config.ts` 第 12-13 行：`serif: ['var(--font-plex-serif)', ...]`，同一条 2026-09-13 注释。
- 全仓库 `grep -rn "Fraunces" app/ components/ lib/`：**零命中**（只在这份 DESIGN-BRIEF.md 的历史记录文字里还提到这个名字）。

**②当时"卡住待拍板"的记录是 2026-09-12 写的，字体切换是 2026-09-13 定案+当天落地的**——PENDING-DECISIONS 里那条记录本身没错（当时确实两份文档打架，确实没人敢擅自换字体），但它只是一天的快照，隔天 Remy 拍板"Artifact Version 10"整个方案（含 IBM Plex Serif）之后，这件事就已经有了权威结论，只是从没人回头把 ② 标记成"已解决"或者更新这份 DESIGN-BRIEF.md。

**结论：不需要 Remy 再表态，字体已经是 IBM Plex Serif，且全站一致（14 处）。上面 v3-v8 所有提到"Fraunces"的段落全部作废，只留作历史论证参考（比如"衬线体只用在数字不用在中文"这条纪律本身依然成立，只是字体名从 Fraunces 换成了 IBM Plex Serif，纪律没变，字体名变了）。**

### ③ 行程切换器——已解决，`trip-switcher.tsx` 已经不存在，现行是 `trip-header-nav.tsx` 的两层结构

**这次任务描述里给的文件路径 `app/trips/[tripId]/trip-switcher.tsx` 本身已经不存在**——2026-09-16 第十七轮（PENDING-DECISIONS 第 742-789 行）把它和 `nav-links.tsx` 合并重写成了 `app/trips/[tripId]/trip-header-nav.tsx`，原因是 Artifact 的 `.topbar-row → .dropdown-panel → .navtabs` 三层在真实 DOM 里必须是同一个 flex-col 容器里的纵向兄弟节点（下拉展开是把下面内容推开，不是浮在上面），旧的 `absolute` 悬浮实现在手机宽度下会溢出裁切。

**现行代码结构**（`trip-header-nav.tsx` 第 119-151 行）跟任务描述的"18px 粗体撑起标题份量"不是同一回事：
- 真正的页面标题是一个独立的静态 `<h1 className="text-[15px] font-bold text-ink">{tripName}</h1>`（第 127 行），这是 2026-09-14 第七轮加的，"标题"这个角色已经从切换器胶囊身上拆出来了。
- 切换器本身现在只是一颗小胶囊按钮：`className="... rounded-full bg-ink px-[9px] py-1 text-[11px] font-medium text-white ..."`（第 144 行），11px，跟"18px 粗体"完全不是一个数字。旁边的代码注释写着"Artifact `.trip-pill{padding:4px 9px;font-size:11px;font-weight:500}` 逐值核对过"——现在这颗胶囊是严格按 Artifact V10 的字面规格做的，不是当年那版"撑起标题份量"的过渡态实现。

**时间线还原**：任务描述引用的"2026-09-12 Remy 真机验收后要求改成 18px 粗体"确实发生过，但那是 2026-09-13"Artifact Version 10"被拍板之前的一次临时调整（当时还没有现在这套"h1 单独当标题"的结构）。09-13 之后，"标题"这个职责被架构性地移交给了新增的静态 h1，切换器胶囊退回到 Artifact 规格本来的"次要触发器"体量（11px）。这不是走了一圈又改回旧值，是产品结构本身变了——现在页面同时有一个真正的 h1 标题和一个功能性切换按钮，18px 粗体那版本是"没有独立 h1 时，切换器胶囊被迫兼职撑标题视觉份量"的权宜设计，这个前提在 09-14/09-16 两轮架构重做后已经不存在了。

**结论：不需要 Remy 再表态。当前实现（静态 h1 15px + 切换器胶囊 11px）已经是产品架构演进后的最终态，且逐值核对过 Artifact V10 字面规格，跟"更早的配色预览稿要 9px 深色小胶囊"这个说法的差距，本质是"胶囊现在不再兼职当标题"这件事解决了，不是两个方案在打架。**

## 二、这份文档之外，还发现一个更大的、没人问起的"悬案"：色板整体已经从暖色换成灰阶，从没写回本文档

这条不在任务给的②③清单里，是体检过程中读 `tailwind.config.ts`/`app/globals.css` 时发现的，重要程度不低于②③，一并报告：

**2026-09-11（候选 D 落地）+ 2026-09-13（Artifact V10 拍板）之后，色板已经从 v3-v8 描述的"remy-thailand 暖色系十色"整体换成了一套近乎单色的灰阶系统**，且这套灰阶恰恰是 v5-v7 三轮反复举证"不能学 team-board/brand-pm/suimuse，因为团队工具的克制灰阶配色跟私人记账本的定位不符"里被否决的那种气质：

| Token | v3-v8 文档写的值（暖色系，已废弃） | 当前代码真实值（`tailwind.config.ts`） | 变化 |
|---|---|---|---|
| `ink` | `#1A1A2E` 近黑藏青（后来柔和化成 `#23232E`） | `#373736`（Black Olive，中性深灰） | 从"带蓝紫色相的深色"变成纯灰 |
| `paper` | `#FEFCF7` 暖米白 | `#F7F7F6`（几乎无色相的浅灰白） | 暖度基本抽掉 |
| `sand` | `#EDE8DA` 暖米灰 | `#DBDAD6`（冷灰） | 同上 |
| `gold` | `#C9A24B` 真实哑金色 | `#A4A3A0`（Mountain Mist，一个灰色，不是金色） | **这个 token 名字还叫"gold"，但已经不是金色，是灰色** |
| `gold-dk` | `#8B6914` 深金字 | `#6E6E6C`（Welded Iron，跟 `muted` 共用同一色号） | 同上，也不是金色了 |
| `coral` | `#E8554E` | `#991B14` | 值变了，语义位置不变（警示色） |
| 财务语义色 | `emerald-600`(该收) / `red-600`(该付) | `positive #3D6141` / `negative #A63926`（+各自 `-dk` 深底变体） | Token 从泛用 Tailwind 色号换成专属命名色，且**全站 0 处再出现 `emerald-600`/`red-600`**（grep 验证过），v3-v8 反复强调"这两个色号最不该乱换"这条纪律已经被绕过，不是被违反——是换了名字之后原纪律字面上就够不到新 token 了 |
| `seafoam`/`sf-lt` | `#2DAA85`/`#D0F0E5` | 代码里还在（`tailwind.config.ts` 第 64-65 行），但目前项目里没找到实际渲染引用 | 可能是历史遗留没清理，不确定还在用 |

**为什么这条重要，值得跟 Remy 单独确认一次**：v5、v6、v7 三轮分别用几乎一模一样的论证，逐一否决了 brand-pm 的青铜色、team-board 的墨绿色、suimuse 的近黑灰调——每次的核心理由都是"trip-expense-ledger 是'有温度的私人记账本'，不是'团队协作看板'，克制冷色调的气质跟定位不符"。但 2026-09-11 的"候选 D"选择，落地效果上跟这三次被否决的方向殊途同归：`ink`/`gold`/`gold-dk`/`sand` 四个最高频出现的 token 现在全部是不同深浅的灰，色相差异被压缩到几乎看不出来，色板里唯一还有真实"颜色"（非灰阶）的只剩下 `positive`/`negative`（财务）和 `ok`/`live`（状态，低频），主色板本身已经不暖了。

这次色板替换是从"候选 A/B/C/D 并排对比稿"里直接选出来的（不是像 v3-v8 那样经过一轮轮论证"为什么暖色/为什么不学team-board"），**这更像是一次视觉偏好的直接拍板，而不是经过跟 v5-v7 同等严谨程度的产品定位论证**。不确定这是 Remy 刻意用"候选 D"推翻了 v5-v7 那套"要有温度"的结论（比如她后来觉得灰阶更耐看/更专业），还是当时只是在比"哪个候选看起来顺眼"、没意识到候选 D 实际上正是 v5-v7 论证过要避开的那种气质。**这条我不替 Remy 拍板，只如实报告这个矛盾——如果 Remy 确认候选 D 灰阶就是她现在真正想要的方向，那么 v3-v8 里"必须有温度/不能像团队看板"这几段论证需要被正式标记废止；如果她其实没意识到这个矛盾、更想要色板里保留多一点真实色相（哪怕只是把 `gold`/`gold-dk` 这两个"名不副实"的 token 换回真正带一点暖黄色相的灰金色，而不是纯中性灰），这值得单独跟她核实一次，成本很低（不需要重新设计，只是在现有灰阶基础上给 `gold`/`gold-dk` 两个 token 找回一点点色相，其它维持不变）。**

## 三、"感觉有点卡"——三个可验证的候选根因（不是空泛的审美判断）

Remy 这次没有点名具体元素，只说了"整体感觉"，下面是我逐代码读出来的三个具体候选，按可信度排序，供后续核实，不是定论：

### 候选 1（最可能）：色板从暖色整体换成灰阶后，"视觉温度"被抽掉，跟上面第二节是同一件事

v3-v8 花了五轮篇幅论证"这个产品需要看起来有温度、不能像团队工具"，这个判断的落地依据（remy-thailand 暖色系）已经被替换成一套近乎单色的灰阶。"卡"这个词除了"局促/不流畅"，也可能在描述"这个东西看起来有点闷、不好看、没有个性"——如果是后一种意思，最直接的候选就是这套灰阶色板。**建议**：不用重新设计，先做一个最小实验——把 `gold`/`gold-dk` 这两个 token 从纯灰（`#A4A3A0`/`#6E6E6C`）换回带一点点暖黄色相的"灰金色"（比如往 v3-v8 那套 `#C9A24B`/`#8B6914` 的色相方向挪 15-20%，不用挪到底），只影响这两个高频出现的强调色（行程标题、分节标题、次要链接），其它 token 不动，成本很低，看 Remy 反馈是否有改善。

### 候选 2（次可能）：36 轮"越改越小"的棘轮效应，没有一轮往回验证过"是不是已经太挤"

通读第四版到第三十六轮的历次改动记录，字号/padding 几乎每一轮只有一个方向——往更小改（Hero 数字 48px→30px→24px；h2 标题 14px→12.5px→10px→9px；控件 padding 从 12px 一路收到 5-7px）。**每一轮的验证问题都是"这样够紧凑了吗"，没有一轮反过来问过"这样是不是已经太挤、需要往回松一点"**。紧凑本身不是问题（`user_compact_ui_preference.md` 明确要紧凑，这个项目现在的密度事实上比那条基准记的默认值还更紧，比如按钮 11px/控件 padding 5-7px，基准默认值是卡片标题 12.5px/chip padding 3-9px），但"卡"这个词如果是"局促、不透气"的意思，36 轮单向棘轮式收紧是一个合理怀疑对象。**建议**：下一轮 ui-auditor 走查真实"🇭🇰2026香港"行程时，除了逐 token 核对 Artifact 数值是否吻合，额外加一步单独的"通读一屏，不测量任何数字，凭直觉打分：这一屏挤不挤"，这是 token 核对测不出来的东西——36 轮里从没有人做过这一步。

### 候选 3（较低可能，但有具体代码证据）：几处"未完工边角"堆积在一起造成不精致的整体印象

原生未样式化的收据上传按钮（见下面第五条反馈）、汇率比价目标币种选项有限（见第一条反馈）、钱包与支付方式概念重叠导致空状态没有解释（见第二条反馈）——这三个具体问题本身都不严重，但堆在一起可能造成"这个产品还没打磨完"的整体印象，跟"卡"这个词的另一层含义（不流畅、有摩擦点）对应。这条不需要单独验证，处理下面五条反馈本身就是在处理这个候选。

## 四、关于第四条反馈（"副标题比标题字号大"）的诊断：不是 h1/h2，是另一种视觉权重错位

实读全站 `<h1>`/`<h2>` 字号（`grep -rn "<h1\|<h2" app/ --include="*.tsx"`），**结构性关系完全正确**：全站 h1 统一 `text-[15px]`（15 处），h2 统一 `text-[10px]` 或 `text-[9px]`（payment-methods 用 9px，其余用 10px），15 > 10/9，标题确实比副标题大，跟 Remy 手动核对的结论一致。

所以这个"感觉"大概率不是字面意义上"某个 h2 的 px 值比同屏 h1 大"，而是**视觉权重**（字重/颜色/serif装饰）造成的错位感，给两个具体候选，供下一轮实读设计+截图核实，我不替 Remy 拍板哪个是真凶：

1. **数字权重系统性地压过标题权重**：Hero 净额数字 `text-2xl`(24px) + IBM Plex Serif + `tabular-nums`，几乎是任何一个 h1(15px) 的 1.6 倍大；钱包卡/比价卡的数字也普遍用 serif + 加粗。这套"数字最重"的设计意图本身是对的（余额优先），但如果"标题"在用户感知里其实包含"这个卡片里最显眼的那个东西"（也就是数字），而不是严格对应语义 `<h1>`/`<h2>` 标签，那么"副标题（数字所在的说明文字）显得比标题（h1/h2 文字）更抢眼"这种错觉是有代码依据的，即使技术上 h1/h2 的 px 值一直是对的。
2. **`trip-header-nav.tsx` 的 `.navtabs` 选中态视觉权重可能盖过下面紧跟着的 h2 分节标题**：导航 tab 选中态是"深色实底 pill + 白字 11px font-medium"（`trip-header-nav.tsx` 第 238 行），跟下面紧跟出现的 h2（10px/9px，细字重，`gold-dk` 文字色，无背景）放在同一屏里对比，前者的"色块+对比度"营造的视觉重量明显高于后者的"细小灰字"，虽然 11px 本身没有比 10px/9px 大很多，但因为有实底色块包裹，视觉上可能读作"这个更重要/更大"。

**建议**：下一轮如果要处理这条反馈，先让 ui-auditor 用真实"🇭🇰2026香港"行程截图（不是量 px，是看整体视觉重量分布），标出用户第一眼落在哪、第二眼落在哪，倒推出"标题感"到底被什么元素抢走了，再决定要不要调（比如给 h2 加一点点背景色块提升权重，或者给 Hero 数字降一点视觉重量），这份简报不替这步下结论。

## 五、五条反馈的具体方向建议

### 1. 汇率比价卡片"目标币种"下拉选项太少——建议：这是历史多轮验证后刻意钉死的固定 5 项，扩大有明确可行路径，但不建议无限展开

**现状**（`lib/fx/fx-compare-defaults.ts` 第 20 行）：`TARGET_CURRENCY_CANDIDATES = ['THB', 'USD', 'SGD', 'CNY', 'HKD']`，固定 5 项，注释明确写着"2026-09-17 第二十轮真机截图坐实的回归教训"——这不是随手写死的，是之前尝试过"从 FX_RATES 表 key 动态推导"，结果因为 HKD 自己换自己的边界情况炸出过真实 bug（PENDING-DECISIONS 第 512 行/644 行两轮都在处理同一类回归），后来才改回固定清单。**这次不建议直接改回"动态推导"这个已经证明有坑的方向。**

但有一条这次体检才确认的新事实：`fx-compare-card.tsx` 顶部注释显示，中间汇率来源在 round33 之后已经从静态 `FX_RATES_FALLBACK` 表升级成实时抓取的 `/api/trips/{tripId}/fx-mid-rates`（源头是 `open.er-api.com`，支持的币种远不止 THB/USD/SGD/CNY/HKD 这 5 个）。**这意味着"目标币种只能是这 5 个"这条限制，技术上已经不再是"汇率数据只覆盖这几个币种"造成的硬约束，纯粹是 Artifact V10 静态方案本身的候选清单没写更多。** 建议：把 `TARGET_CURRENCY_CANDIDATES` 从 5 项扩到一个仍然是**人工精选、写死的**清单（不要变回动态推导，避免重蹈 round19/20 覆辙），根据 Remy 实际去过/常去的目的地补齐几个明显缺失的常见旅行币种（比如 JPY/EUR/GBP/AUD/KRW，具体加哪几个建议跟 Remy 确认她近期行程目的地），"我持有"候选清单（目前 MYR/USD/HKD/CNY 4 项）可以维持不变或酌情跟进，两边不必同步扩张。

### 2. "我的钱包"区块显示空白——建议：这不是要修的 bug，是空状态文案的产品沟通问题

D1 数据已确认这条真实行程 wallet 表是 0 行，空白是正确行为。但正如任务描述指出的，"钱包"（行程级、私有余额追踪、用户要主动新建）和"支付方式"（账号级、跨行程共享的信用卡/现金配置）是两个高度相似又完全不同的概念，界面上只呈现"空白+一个新建入口"，没有解释这个区别，容易让用户以为"支付方式都配置好了，为什么钱包还是空的"。**建议**：零钱包状态下，在"+新建钱包"卡片旁边加一行极简说明文字（走 `text-[10px] text-muted` 这个既有 token，不需要新样式），大意是"钱包用来追踪你在这趟行程里实际有多少现金/余额，跟你账号里配置的支付方式是两回事"——这个措辞需要 Remy 或 frontend-dev 打磨，这里只给方向，不代拟最终文案。

### 3. 结算页面感觉还是跟方案不符——按任务要求，这条不处理，留给 Remy 自己逐屏核对

不在这份体检的结论范围内，不重复第三十三轮"逐 token 核对过是对的"这个旧结论，也不重新下判断。

### 4. 字体层级"副标题比标题大"的观感——见上面第四节，两个候选权重错位假设，建议先截图定位再决定要不要动，不建议直接调整 h1/h2 本身（那对已经是对的）

### 5. "记一笔消费"表单原生 `<input type=file>` 样式不搭——建议：包一层复用现成 `.btn-secondary` 触发器，不新造文件选择 UI

**现状**（`expense-form.tsx` 第 522 行）：`<input id="receipt" type="file" ... className="text-[12.5px]" />`，就是浏览器原生控件，字号被改了但按钮本身（"选择文件"/"Choose File"字样、边框、圆角）完全是系统默认，未接入 `ink`/`sand`/`rounded` 这套系统的任何 token，是这次通读代码里最明显的一处"零改造"控件。**建议**：不用发明新的文件上传组件，用一个隐藏原生 `<input type=file>`（`sr-only` 或 `hidden`，保留可访问性/原生弹窗行为）+ 一个可见的触发器复用已经存在的 `.btn-secondary`（`rounded-full border border-sand ... text-[11px] ...`，`app/globals.css` 已定义），按钮文字可以是"📎 选择收据"或"上传收据"，选中文件后旁边显示文件名（`text-[10px] text-muted`）。这是这套设计系统里最低成本、复用度最高的做法，不需要新 token。

## 六、下一步建议（不是这份简报的强制项，供 lifeos-pm/Remy 参考排期）

1. **文档层面**：建议找一次专门的时间，把这份 DESIGN-BRIEF.md 的 v3-v8 部分做一次"标记废止但不删除"的整理（比如统一加删除线或移到文末"历史存档"区），避免下一个读这份文档的人再次被过时内容误导；这次体检只在文档最前面加了一条提醒横幅，没有动 v3-v8 正文本身（怕删改后丢失里面仍然成立的论证逻辑，这个判断留给 Remy 或下一轮任务决定要不要做更彻底的整理）。
2. **色板层面**：建议单独找 Remy 确认一次"候选 D 灰阶是不是她现在真正想要的方向"（见上面第二节），这条如果不确认，未来任何人再被要求"参考 XX 系统改颜色"都会重复同一个"暖色 vs 灰阶"的拉锯。
3. **密度层面**：下一轮 ui-auditor 走查建议加一步"不量 px、凭直觉判断挤不挤"的整体观感检查，别只做逐 token 核对（见上面候选 2）。
4. **本次五条反馈**：目标币种候选扩充（需 Remy 定具体币种清单）、钱包空状态加说明文字、文件上传按钮改造，三条都是低成本、可以直接排进下一轮实现任务；字体层级问题建议先截图定位再动手，不建议直接猜测调整。
