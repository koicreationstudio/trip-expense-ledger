...
- 本体域名是 `*.workers.dev`（`https://trip-expense-ledger.remybali.workers.dev`），不是 `*.pages.dev`——这是 Worker 部署（`wrangler deploy`）跟 Pages 部署（`wrangler pages deploy`）的天然差异
- **主要入口网址是 `https://kongsi-trip.pages.dev`（2026-09-25 起）**，`remybali.workers.dev` 那个网址保留不动当备用入口。起因是马来西亚网络挡 `*.workers.dev` 域名，Remy 手机不开 VPN 打不开旧网址会白屏，`*.pages.dev` 域名不受影响；另外她不想网址里带 `remybali` 字样。`kongsi-trip` 不是另一份代码，是 `pages-proxy/` 目录下一个独立的 Cloudflare Pages 项目，靠 service binding 把收到的请求原封不动转给这个 Worker 本体（同一份 D1 `trip-expense-ledger-db`，没有新建数据库/复制数据）。转发层本身没有业务逻辑，也没有自己的 UI/账号体系——两个网址打开的是同一个 app、同一批数据，只是入口不同。详见 `pages-proxy/CLAUDE.md`，部署走 `pages-proxy/deploy.sh`（跟这个 `deploy.sh` 是两个独立脚本，改这个 Worker 本体代码只需要跑这份 `deploy.sh`，不需要跑 `pages-proxy/deploy.sh`；只有转发层自己的文件改了才需要跑那份）。
