...
## 【2026-09-19，第二十七轮，Remy 8 张截图逐条处理：3 条实测不成立（已对齐）+ 3 条真bug已修+1条顺手清理安全残留+1条圆角矛盾交还表态，新 session 从这里读起】

背景：Remy 这轮发了 8 张截图，逐条要求核对。开工前先读了这份文档最新记录 + `PARITY-CHECKLIST.md`，确认了 round18 圆角决定、round25 的按钮/结算修复、2026-09-18 的安全事故处置情况。这轮**每一项都先用真实 Playwright 脚本实测线上 computed style，不是先入为主照 Remy 描述直接改代码**——这个纪律救了三条：实测下来发现 3 条截图描述的问题在当前代码库/线上实际检查中并不成立，如实报告，没有为了"看起来在干活"硬改已经对的代码。

### 零、顺手发现并清理的安全残留（不在这轮任务范围内，但碰到了如实处理）

工作树里有一个未纳入 git 版本控制的目录 `audit-diffs/round26-full-reverify/`（2026-09-18 13:29 左右生成，某次未被写进 PENDING-DECISIONS 的走查留下的），里面的 7 个脚本文件（`measure.js`~`measure7.js`）硬编码了一个真实的 `tel_session` cookie 明文 token。查证：
- 这个 token 对应的 `session` 表记录**已经不存在**（`wrangler d1 execute` 查 `session` 表全表 13 行，没有一行匹配这个 token 的 hash 模式/marker），说明这个验证 session 之前测完确实按规矩清理过，这份残留只是**脚本文件本身没删**，不是活跃可用的登录凭证。
- 没有提交过 git（`??` untracked），没有 GitHub 侧暴露风险。
- 已经把这 7 个脚本文件删掉（`data/` 和 `screenshots/` 里的截图/JSON 数值记录留着，没有敏感 token，作为上一次走查的参考数据保留）。

这不是这轮任务要求的工作，是核对代码时顺手发现的，按 2026-09-18 那次安全事故的处置原则（"验证脚本别到处留真实token"）顺手清理，如实记录不藏着。

### 一、实测后发现"已经对齐"、这轮没有改代码的 3 项（附实测证据）

**1. "管理行程"切换面板紧凑度**——用 Playwright 脚本精确抓取真实 DOM 里那个面板容器（不是靠肉眼估读源码），实测：外层 `padding:4px / border-radius:14px`，"管理行程"标题行 `padding:2px 6px 3px`，行内 `padding:4px 6px / gap:6px`，选中行 `background:#EBEAE8 / border-radius:8px`——逐项跟 `reference/artifact-v10-source.html` 第130-144行 `.dropdown-panel`/`.dd-head`/`.dd-row` 的字面数值核对，**全部精确一致，没有发现任何偏差**。`trip-header-nav.tsx` 这块代码本身就是照方案字面值写的（round17 就已经对齐）。

**2. 汇率比价标题"→XXX"没跟着目标币种更新**——用 Playwright 脚本实测：打开汇率比价卡片，标题显示"💱 汇率比价 → THB"；点开"目标币种"下拉选"USD 美金"；标题**立刻变成**"💱 汇率比价 → USD"。这是真实 React 状态驱动的动态文字（`fx-compare-card.tsx` 第 359 行 `💱 汇率比价 → {effectiveTarget}`，`effectiveTarget` 是 state），不是写死文字，实测确认切换生效。

**3. 汇率渠道卡片（Wise/TNG/ATM/换钱店/支付宝）间距不够紧凑**——用 Playwright 脚本实测：每张卡 `padding:8px 10px / border-radius:14px / font-size:11px`，列表卡片间 `gap:7px`——逐项跟 `.fx-row{padding:8px 10px}`/`.fx-channels{gap:7px}`（源码第221/220行）核对，**全部精确一致**。

这 3 项都是"如实说查证不成立"，不是敷衍不修——每一项都有真实 Playwright 脚本的实测数字为证（不是只读源码假设），跟过去几轮"读代码觉得应该对"就打勾的教训不一样。**可能的解释**：Remy 这批截图有可能是之前某一轮（round17/18/20/22/25 都碰过这几块）修复生效之前拍的旧截图，或者手机端因为 iOS Safari 自动文字缩放（"webkit-text-size-adjust"）让画面看起来比实际 CSS 数值更松散——这条顺手补了个低风险的全局防御（见下方"三"）。如果 Remy 现在重新用真机看还是觉得不紧凑，需要她提供具体是哪个元素/截图当下的真实使用场景（Safari书签打开 vs 主屏幕图标打开），这轮的实测数字摆在这里，不能再无凭据地重改。

### 二、实测坐实、这轮真的修了的 3 项

**4. 快速记账卡缺日期字段（新功能，方案原本就没有，如实确认不是方案缺口）**——查证 `reference/artifact-v10-source.html` 的 `.quickadd` 相关标记，确认方案这张卡本来就只有分类/币种/金额/商家名称/分摊，没有日期字段，这是 Remy 现在想要的新功能。`app/trips/[tripId]/quick-add-expense.tsx` 新增日期输入框（`type="date"`，默认今天，可以改成任意日期方便补记），位置在商家名称字段下面，样式跟卡片其它字段一样是 `field-input-dark`。提交时把这个日期传给后端（原来固定写"现在这一刻"，现在跟着这个字段走）。真实线上实测：`<input type="date">` 存在，`value` 默认是当天日期（"2026-09-19"），`class="field-input-dark"`，字号 10px，跟卡片其它字段视觉一致。

**5. 活动流缺约算金额 + 支付方式没图标——两处都是真的（这条查证过程发现了一个值得记录的产品判断，不是完全照抄 Remy 描述）**。
- **约算金额**：查 `expense-list.tsx` 原来的逻辑是"这笔消费自己的币种 ≠ 这趟行程本位币才显示约算成本位币的小字"。查了 Remy 这条真实行程的 D1 数据——**11 笔消费全部是用 HKD 记的，本位币也是 HKD**，两者永远相等，这个触发条件在这条真实数据下**从来不会成立**，这正是她反馈"完全没有这个信息"的根因，不是显示逻辑漏做了，是触发条件设计跟真实数据状态对不上。
  **这里有一个我做的产品判断，如实记录，不是悬案但要让 Remy 知情**：没有简单地把触发条件从"跟本位币比"改成"总是显示"（那样 100% HKD 记账的行程，约算数字会跟原数字完全一样，没有信息量，纯粹多一行占地方）。看了 Artifact demo 的实际语义——demo 里行程本位币是 ฿(THB)，约算目标固定是 RM(MYR)，这两个从来不是同一个东西，约算的意义是"这笔钱换算成 Remy 自己最熟悉的马来西亚令吉是多少"，不是"这笔钱换算成这趟行程自己的本位币是多少"（后者对她没有额外信息量，因为她本来就在用本位币记账）。这个 app 全站的汇率基建（`lib/fx/rate-cache.ts`）本来就是以 MYR 为中心基准，所以这次把约算目标改成固定的 MYR：只要行程本位币不是 MYR，每一行都会显示"≈RM xxx.xx"（服务器端用行程本位币金额乘以实时 MYR 汇率算出来，走的是已有的 `ensureMyrRatesFresh`/`getMyrRateSnapshot`/`deriveMidRate` 这套基建，没有另起一套汇率逻辑）；本位币恰好就是 MYR 时才不重复显示（那种情况显示"≈RM xxx"会跟原数字一模一样，没有意义）。
  真实线上实测：11 笔真实消费全部显示出"≈RM xxx.xx"（比如"雪糕 HK$26.00 ≈RM 13.53"），数字用的是当下实时汇率算的，不是编的。
- **支付方式图标**：`payment_method` 数据模型只有两种 `kind`（`card`/`cash`，没有"支付宝"这种更细的专属类型字段，"支付宝"只是自由文本 label）。新增 `paymentMethodIcon()` helper：label 文字里认出"支付宝"用方案demo同款的🅰️，否则按 kind 给💳(card)/💵(cash)，两者都推断不出来（比如显示"其他人的支付方式"这种没有 kind 信息的情况）就不加图标只显示原文字，不瞎猜一个可能错的图标。真实线上实测：这条真实行程里配置了"现金"支付方式的几笔消费，meta 行正确显示"💵 现金"。

**6. 换汇记录空状态提示字号偏大（真bug，已修）**——`exchange-record-list.tsx` 第63行原来用 `text-sm`（14px），方案 `.empty{font-size:11.5px}`。真实线上实测确认之前确实是14px，改成 `text-[11.5px]`。**顺手发现并一起修了同一类问题**：`expense-list.tsx` 第159行"还没记过账"的空状态提示也是同样的 `text-sm`（14px）bug，同一套 `.empty` 规格，一起改成 11.5px（这条真实行程有11笔消费不会触发这个空状态，改动是代码层面确认对的，没在这条真实行程上直接看到效果，如实说明）。

### 三、顺手加的一个低风险防御性改动（不确定能不能解决"看起来不紧凑"的根本原因，如实说明局限）

`app/globals.css` 补了 `-webkit-text-size-adjust:100%`（全局，`html` 选择器）——iOS Safari 有一个"自动文字缩放"的历史行为，在窄屏多行文本块上会自动放大渲染字号，这个放大**不会**反映在 `getComputedStyle().fontSize` 上（所以 Playwright 测不出来），但真机上肉眼看会觉得"字看起来比数值大、没那么紧凑"。这是不是本轮 3 项"实测已对齐"截图看起来不紧凑的真正原因，**没有办法验证**（跟 round25 iOS 防放大那条一样，无头浏览器复现不了真实 Safari 渲染行为），只是一个低风险、没有副作用的通用最佳实践修复，顺手加上，不确保能解决 Remy 看到的问题。

### 四、第七条——圆角矛盾，如实摆出来问 Remy，这轮没有动这颗按钮

Remy 这轮反馈"记一笔消费"悬浮按钮"说了很多次还是没改成和方案一样"（方案字面是 `border-radius:999px` 完全圆形胶囊）。**但 round18（2026-09-13）Remy 自己明确反馈过**方案的完全圆形胶囊"看起来像浮起来"，要求收成现在这个圆角矩形 `rounded-xl`（12px），round25 也再次确认并保留了这个决定（实测线上确实是12px，`background:rgb(55,55,54)`，其它 padding/字号/gap 都已经跟方案字面对齐，唯独圆角是round18的决定没有跟方案走）。

**这两条反馈直接矛盾，这轮没有替她翻案，也没有假装没看到矛盾**——需要 Remy 明确表态：
- 是想推翻 round18 的决定，改回方案的完全圆形胶囊（999px）？
- 还是这颗按钮其实是别的地方感觉不对（比如内边距、字号、阴影、位置），圆角本身不用动，只是她记混了/描述不准确？

这轮**没有改这颗按钮的圆角**，等她回答后再动手。

### 五、验证

**真实行程**：全程"🇭🇰2026香港"（`f78a6b5e-8612-4097-8bfd-88a5db664045`），Remy 真实身份链接登录（`https://trip-expense-ledger.remybali.workers.dev/id/aNhfVNPU7ZGosHWFmdLfp5WtUxB_QGBqjNldoMGqaWA`），未使用测试行程。这条真实行程当前 11 笔消费全部HKD，0钱包，0换汇记录（跟之前几轮结束时状态一致）。

**我自己直接写脚本验证**：`wrangler d1 execute --remote` 插入一条独立标记的验证 session（`user_agent='pm-round27-screenshot-fixes-verify-2026-09-19'`），Playwright（WebKit 引擎，iPhone 视口）注入 `tel_session` cookie 标准登录，逐项实测 computed style（详见上面各条），测完 `DELETE` + `SELECT count(*)` 确认归零，未在真实行程留下任何测试脏数据。

**独立 ui-auditor（跟做实现的不是同一个agent）**：用身份直连链接登录真实账号，走的是真实行程"🇭🇰2026香港"（同一条），桌面(1280×900)+手机(390×844)两种视口。结论：三项改动（日期字段/≈RM约算+支付方式图标/换汇空状态字号）代码实现跟真机表现完全一致；日期字段默认今天、改日期不触发提交；11笔真实消费全部正确显示"≈RM xx.xx"，带现金支付方式的正确显示"💵 现金"（如实说明：这条真实行程没有支付宝/银行卡记录的消费，🅰️/💳 两种图标没能用真实数据肉眼验证，代码逻辑审查没问题）；换汇空状态11.5px字号跟旁边"仅自己可见"字号比例协调。顺手复核的三项（管理行程面板/汇率比价标题动态/渠道卡片间距）也确认正常。全程只读未提交任何真实表单。Console 0 error（只有跟这轮改动无关的字体preload历史warning）。截图 `audit-diffs/round27-screenshot-fixes/`（10张——**这批截图 ui-auditor 一开始直接存去了 `~/Desktop/Claude/` 根目录，不是 trip-expense-ledger 自己的仓库，重演了 2026-09-18 安全事故复盘那条"验证产物别写进别的项目目录"的同类问题，虽然这次只是截图不含密钥/token，风险比那次低很多，但确认是同一个坏习惯的复发，这轮已经手动搬回本项目 `audit-diffs/` 目录，如实记录不掩盖，以后指派 ui-auditor/子agent 做真机验证时要更明确指定截图存放路径**）。

**代码验证**：`npm run typecheck`/`npm run lint` 全过；`npm test` 78个单测全过（没有新增测试，这轮改动都是纯展示逻辑，复用现成的 `ensureMyrRatesFresh`/`deriveMidRate` 等已经有单测覆盖的基建，没有引入新的计算逻辑需要单独覆盖）；`./deploy.sh` 五关全过（跑了两次——第一次落地4项代码改动，第二次补一条 `globals.css` 的 `-webkit-text-size-adjust:100%` 防御性CSS），最终线上 Version ID `e5690007-959a-46c9-9f19-4a271ae8ce95`，回读 `/api/health` 200。

**git**：commit 和 push 分开执行。
