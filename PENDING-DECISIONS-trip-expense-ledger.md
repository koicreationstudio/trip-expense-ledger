...
- `./deploy.sh` 五关 + 部署 + 生产回读（commit/Version ID 见下方，跑完补）。
- **独立 ui-auditor 生产真机走查，用 Remy 真实行程「🇭🇰2026香港」（`trip_id=f78a6b5e-8612-4097-8bfd-88a5db664045`）**：
  1. 新增功能——「设置当前余额」面板确认「现金 USD」这行以灰显+「建钱包」按钮出现（这条真实行程当前只有「现金」HKD 一个真实钱包，D1 查证过）；点「建钱包」确认真的建出来；建完这一行能填余额并保存成功；确认原有 HKD 钱包行完全没受影响（还能正常填余额保存）；灰显行视觉跟面板里现有行样式是否统一。
  2. 冷启动路径——尽量多次从"刚登录/行程列表点进"这条路径复测，如实报告有没有复现间歇性失败（不预设"这次肯定修好了"，如实记录复现/未复现的次数和场景）。
  3. 顺手检查一下结算页这次是否也不再出现 Remy 之前反馈的"未按方案更新"症状（round43 提出的待验证猜想，不是这轮任务本身的范围，只是顺手核对不需要额外开一轮）。
  - 走查结果、截图路径、测试用 session 清理记录，走查完成后补进本节。
